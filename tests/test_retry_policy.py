...
This server fans out to dozens of item requests per tool call, which makes the
jitter matter more here than elsewhere: without it, a rate-limited burst retries
in lockstep and arrives back as one wave.
"""
...
def test_cap_binds_after_the_jitter() -> None:
    ...
    assert max(deep) <= server.RETRY_MAX_DELAY

    hinted = _retry_after_error("600")
    assert {server.compute_delay(1, hinted) for _ in range(100)} == {server.RETRY_MAX_DELAY}
